...
class DLRSLoginService(private val config: DLRSConfig, private val dataService: PlayerDataService) {
    ...
    private fun startPolling(session: LoginSession) {
        ...
        // 设置超时任务（从配置读取，默认 60 秒）
        val timeoutEnabled = config.isLoginTimeoutEnabled()
        val timeoutSeconds = config.getLoginTimeoutSeconds()
        val timeoutTicks = timeoutSeconds * 20L // 转换为 tick
                ...
                Bukkit.getPluginManager().getPlugin("DLRS-GAS-For-Minecraft")!!,
                {
                    ...
                    if (loginSessions.containsKey(player.uniqueId)) {
                        ...
                        Bukkit.getScheduler().scheduleSyncDelayedTask(
                            ...
                            {
                                if (player.isOnline) {
                                    player.kickPlayer("§c[DLRS-GAS] 登录超时\n\n请重新尝试登录")
                                }
                            },
                            1L
                        )
    ...
    private fun checkLoginStatus(session: LoginSession) {
        ...
        if (response != null && response.getInt("code") == 200) {
            ...
            if (userInfo != null) {
                ...
                session.player.sendMessage("§a[DLRS-GAS] §7用户 ID: §f${userInfo.uid}")
                ...
                val bindResult = dataService.checkAndBindAccount(
                    userInfo.uid,
                    session.player.uniqueId.toString(),
                    session.player.name
                )
                if (!bindResult.success) {
                    // 绑定冲突，踢出玩家
                    session.player.kickPlayer(bindResult.message)
                    return
                }
     ...
     * 获取绑定到指定 UID 的玩家名称
     */
    private fun getBoundPlayerName(uid: String): String {
        return dataService.getBoundPlayerName(uid) ?: "未知"
    }
